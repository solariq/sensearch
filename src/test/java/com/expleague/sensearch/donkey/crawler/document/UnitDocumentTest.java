...
public class UnitDocumentTest extends SensearchTestCase {
  ...
  public void xMLWithEmptyLinksTest() {
    ...
    Assert.assertEquals(sections.get(0).links().size(), 0);
    Assert.assertEquals(sections.get(1).links().size(), 1);
  ...
  public void xMLwithEmptySectionsTest() {
    ...
    Assert.assertEquals(sections.size(), 6);

    Assert.assertEquals(sections.get(2).title(), Collections.singletonList("test"));
    Assert.assertEquals(sections.get(2).text().toString(), "");

    Assert.assertEquals(sections.get(0).title(), Collections.singletonList("Мужун Вэй"));
    Assert.assertEquals(
  ...
  public void xMLwithoutCategoriesTest() {
    ...
    test.add("Ссылки");
    Assert.assertEquals(sections.get(1).title(), test);
  }
  ...
  public void xMLwithoutIDTest() {
    ...
    Assert.assertEquals(sections.get(0).title(), Collections.singletonList("Тэсо (Тонбуё)"));
  }
  ...
  public void xMLwithoutTitleTest() {
    ...
    Assert.assertEquals(sections.get(0).title(), Collections.singletonList("Ши Чжи"));
  }
  ...
  public void xmlWithSubcategoriesTest() {
    ...
    Assert.assertEquals(Arrays.asList("Ссылки"), sections.get(0).title());
    Assert.assertEquals(Arrays.asList("Ссылки", "Ссылка1"), sections.get(1).title());
    Assert.assertEquals(Arrays.asList("Ссылки", "Ссылка2"), sections.get(2).title());
    Assert.assertEquals(
        Arrays.asList("Ссылки", "Ссылка2", "Подссылка"), sections.get(3).title());
    Assert.assertEquals(Arrays.asList("Не Ссылки"), sections.get(4).title());
  }
  ...
  public void xmlWithLinksTest() {
    ...
    Assert.assertEquals(2, sections.get(0).links().size());

    Assert.assertEquals("точимилькококо", sections.get(0).links().get(0).targetTitle());
    Assert.assertEquals(123, sections.get(0).links().get(0).targetId());
    Assert.assertEquals(0, sections.get(0).links().get(0).textOffset());

    Assert.assertEquals("ПуэблА", sections.get(0).links().get(1).targetTitle());
    Assert.assertEquals(-1, sections.get(0).links().get(1).targetId());
    Assert.assertEquals(44, sections.get(0).links().get(1).textOffset());

    Assert.assertEquals("", sections.get(2).links().get(0).targetTitle());
    Assert.assertEquals(-1, sections.get(2).links().get(0).targetId());
    Assert.assertEquals(0, sections.get(2).links().get(0).textOffset());
  }
