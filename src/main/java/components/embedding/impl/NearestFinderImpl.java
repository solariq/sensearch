...
public class NearestFinderImpl implements NearestFinder {
    ...
    public List<String> getNearestWords(Vec mainVec, int numberOfNeighbors) {
        return getNearest(mainVec, numberOfNeighbors, EmbeddingImpl.getInstance().getWordVecMap(), DEFAULT_MEASURE);
    }

    @Override
    public List<Long> getNearestDocuments(Vec mainVec, int numberOfNeighbors) {
        return getNearest(mainVec, numberOfNeighbors, EmbeddingImpl.getInstance().getDocIdVecMap(), DEFAULT_MEASURE);
    }
